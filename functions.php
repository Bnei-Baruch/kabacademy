...
function get_academy_courses()
{
    if (is_numeric($_POST['page']) && check_ajax_referer('load-more-courses', 'security')) {
        ...
        die(json_encode(array('result' => 'OK', 'content' => $return)));
    }
}
...
function namaste_enroll()
{
    ...
    if (!is_user_logged_in()) {
        ...
        if (!is_array($required_lessons_ids)) $required_lessons_ids = array();
...
function namaste_next_lesson_custom($atts)
{
    ...
    if (empty($required_lessons)) return "<a class='" . $class . "' href='" . add_query_arg(array('tab' => 'nolessons'), get_permalink($course_id)) . "'>" . $text . " " . __("(Starts soon!)", 'qode') . "</a>";
...
function update_note_title()
{
    ...
    if (!isset($notes[$_POST['note_id']])) {
        $notes[$_POST['note_id']] = array('title' => __('New Note', 'qode'), 'content' => __('Type text here...', 'qode'), 'date' => $date);
    }
...
function qode_styles_Re()
{
    ...
    wp_enqueue_script("custom-js", get_stylesheet_directory_uri() . "/js/custom.js", array("filereader"), false, true);
...
function update_topic_custom()
{
    ...
    die(json_encode(array('result' => 'OK', 'content' => bbp_get_topic_content($_POST['id']))));
}
...
function update_reply_custom()
{
    ...
    die(json_encode(array('result' => 'OK', 'content' => bbp_get_reply_content($_POST['id']))));
}
...
function like_custom()
{
    ...
    die(json_encode(array('result' => 'OK', 'count' => $count)));
}
    ...
    foreach ($replies as $reply) {
        ...
        <div class="single_topic_reply" data-id="<?php echo $reply->ID; ?>">
            ...
            <div class="content_wrapper">
                <div class="reply_content"><a class="author-link"
                ...
                <div style="display:none" class="reply_content_edit"><textarea
                        class="reply_content_edit_textarea"><?php echo get_post_field('post_content', $reply->ID); ?></textarea><a
                        href="#" class="smiles_open"></a>

                    <div class="edit_actions"><a class="cancel" href="#">Отменить</a></div>
                </div>
                ...
                <div class="actions"><span
                    ...
                    <div class="like-count"<?php if (empty($likes)) echo ' style="display:none"'; ?>><i
                            class="like-img"></i><span class="count"><?php echo (int)$likes; ?></span></div>
                </div>
            </div>
            ...
            <div class="topics_list_single_topic" id="topic-<?php echo bbp_get_topic_id(); ?>" 
                ...
                <div class="single_topic_actions">
                    ...
                    <?php $like = get_post_meta(bbp_get_topic_id(), 'like_' . get_current_user_id(), true); ?><a
                        class="like"<?php echo (!empty($like)) ? ' style="display:none"' : ''; ?>
                        href="#"><?php _e('Like', 'qode'); ?></a><a
                        class="like dislike"<?php echo (empty($like)) ? ' style="display:none"' : ''; ?>
                        href="#"><?php _e('Dislike', 'qode'); ?></a>
                ...
                <div class="single_topic_replies_container">
                                    ...
                                    <div class="actions"><span
                                        ...
                                        <div
                                            ...
                                            <i class="like-img"></i><span
                                                class="count"><?php echo (int)$likes; ?></span></div>
                                    </div>
                                </div>
                                <?php if ($reply->post_author == get_current_user_id()): ?>
                                    <a class="addi_actions_open" href="#"></a>
                    ...
                    <div class="single_topic_reply_form">
                        <form 
                            ...
                            <div class="reply-form"><textarea
                                    placeholder="<?php _e('Введите текст сообщения...', 'qode'); ?>"
                                    name="content"></textarea><a href="#" class="smiles_open"></a></div>
...
function upload_forum_file()
{
    ...
    if ($type == 'post') {
        ...
        $content = '<div class="single_topic_single_attachment">
                <div class="attachment-image"><a target="_blank" href="' . $r[0] . '">' . wp_get_attachment_image($attachment_id, array('32', '32')) . '</a></div><div class="attachment-controls"><a class="delete-attachment" data-id="' . $attachment_id . '" href="#">Удалить</a></div>
            </div>';
    } else {
        if ($id != 0) {
    ...
    die(json_encode(array('result' => 'OK', 'id' => $attachment_id, 'content' => $content)));
...
function addMailChimpSegment($student_id, $course_id, $status)
{
    ...
    $optAddSegment->segment_opts->conditions = array('field' => 'Courses', 'op' => 'contain', 'value' => 'a');
    ...
    curl_setopt($curlAddSegment, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
